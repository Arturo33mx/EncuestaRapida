...
$array = array(
    ...
    "6) Conchita Cazares",
    "7) Julián Peña",
    "8) Pedro Fernández Carrera",
    "9) Daniel Vargas Bello",
    "10) Ignacio Espinosa Pliego",
    "11) Héctor Mendoza Herrera"
);
?>
<table class="table table-bordered table-hover">
    <thead>
        <tr class="bg-info text-white">
            ...
            <th>En caso de negativa, dar información adicional 9.2 - Ayudado</th>
            <th>9.3 - ¿De qué partido político es?</th>
            <th>9.4 - ¿Ud. Votaría por él para DIPUTADO LOCAL?</th>
            <th>9.5 - Y, ¿Qué opinión tiene de él / ella?</th>
        </tr>
    </thead>
    <tbody>
        <tr class="">
            ...
            <td>
                <select class="form-control" onchange="CorroboraTabla(<?php echo $i+1?>,2)" id="cmbPreg92<?php echo $i+1?>" disabled>
                    <option value="0">Selec.</option>
                    <option value="1">Si </option>
                    <option value="2">Creo que si</option>
                    <option value="3">No</option>
            ...
            <td>
                <select class="form-control" id="cmbPreg95<?php echo $i+1?>" disabled>
                    <option value="0">Selec.</option>
    ...
    function CorroboraTabla(id,opc){
        if (opc==1){
            if($('#cmbPreg91'+id).val()==1 || $('#cmbPreg91'+id).val()==2){
                ...
                $('#cmbPreg95'+id).removeAttr("disabled");
                $('#cmbPreg92'+id).val(0);
            }
            else{
                ...
                $('#cmbPreg95'+id).attr("disabled", "disabled");
            }
        }
        else{
            if($('#cmbPreg92'+id).val()==1 || $('#cmbPreg92'+id).val()==2){
                $('#cmbPreg93'+id).removeAttr("disabled");
                $('#cmbPreg94'+id).removeAttr("disabled");
                $('#cmbPreg95'+id).removeAttr("disabled");
            }
