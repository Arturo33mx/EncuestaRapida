...
$consulta="select COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'encuesta_promocion';"; 
...
if($cuantos_registros==0){
	...
	while($MostrarFila=$bd->fetch_array($Resultado)){
        $indx = 4;
        if($ini>6){
            $sql="SELECT `".$MostrarFila['COLUMN_NAME']."` Res, count(*)Total FROM encuesta_promocion
                where `".$MostrarFila['COLUMN_NAME']."` <> 0 and `".$MostrarFila['COLUMN_NAME']."` is not null and EstatusNumero=1
